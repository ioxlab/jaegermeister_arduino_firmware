...
enum ShockPad {
    DONT_SHOCK = 0, SHOCK = 1,
};

enum Level {
    InvalidLevel = 0, Level1 = 1, Level2 = 2, Level3 = 3, Level4 = 4,
};
...
void Func_ShockPads(void) {
    ...
    while (lul_ActualTimestamp < lul_ShockTimestamp) {
        ...
        // check which led should be light up
        for (lu8_PadIndex = 0; lu8_PadIndex < d_NUM_SHOCKPADS; lu8_PadIndex++) {
...
void Func_Trigger() {
    ...
    while (lul_ActualTriggerTimestamp < lul_ShockTimestamp) {
        // check which pad should get shocked
        for (uint8_t lu8_PadIndex = 0; lu8_PadIndex < d_NUM_SHOCKPADS; lu8_PadIndex++) {
            if ((SHOCK == shock_pads[lu8_PadIndex]) && (Level1 == shock_level)) {
                switch (lu8_PadIndex) {
                    case 0:
            ...
            if ((SHOCK == shock_pads[lu8_PadIndex]) && (Level2 == shock_level)) {
                switch (lu8_PadIndex) {
                    case 0:
                        digitalWrite(d_SHOCKPAD_EN_CH1, HIGH);
                        break;
            ...
            if ((SHOCK == shock_pads[lu8_PadIndex]) && (Level3 == shock_level)) {
                switch (lu8_PadIndex) {
                    case 0:
                        digitalWrite(d_SHOCKPAD_EN_CH1, HIGH);
                        break;
            ...
            if ((SHOCK == shock_pads[lu8_PadIndex]) && (Level4 == shock_level)) {
                switch (lu8_PadIndex) {
                    case 0:
                        digitalWrite(d_SHOCKPAD_EN_CH1, HIGH);
                        break;
...
void Func_MeasureSensorAfterShock(uint8_t *au8p_SensorArray) {
    ...
    char response[21];
    sprintf(response, "P1=%d;P2=%d;P3=%d;P4=%d\n", lu8a_SensorArray[0], lu8a_SensorArray[1], lu8a_SensorArray[2],
            lu8a_SensorArray[3]);
